...
# common setup
export BIN=${BIN:-test_pushpull_cache}
...
DMLC_ROLE=server numactl -m 0 python3 $THIS_DIR/$BIN.py $@ 2>&1 | tee server.log &
# DMLC_ROLE=worker python3 $THIS_DIR/$BIN.py $@ &
# export STEPMESH_DROP_RATE=1
export STEPMESH_CPU_START_OFFSET=15
DMLC_ROLE=worker numactl -m 0 python3 $THIS_DIR/$BIN.py $@ 2>&1 | tee worker.log
