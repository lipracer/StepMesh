...
function cleanup() {
    ...
    pkill -9 -f test_whole_cudagraph
    sleep 1
}
trap cleanup EXIT
# cleanup
...
# common setup
export BIN=${BIN:-test_whole_cudagraph}
...
DMLC_ROLE=server python3 $THIS_DIR/$BIN.py $@ 2>&1 | tee server.log &
# DMLC_ROLE=worker python3 $THIS_DIR/$BIN.py $@ &
# export STEPMESH_DROP_RATE=1
# export STEPMESH_CPU_START_OFFSET=15
DMLC_ROLE=worker python3 $THIS_DIR/$BIN.py $@ 2>&1 | tee worker.log
