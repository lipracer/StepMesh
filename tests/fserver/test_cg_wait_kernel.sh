...
# common setup
export BIN=${BIN:-test_cg_wait_kernel}
...
DMLC_ROLE=server CUDA_VISIBLE_DEVICES=0,1,2,3 STEPMESH_GPU=0 python3 $THIS_DIR/$BIN.py $@ 2>&1 | tee server.log &
# DMLC_ROLE=worker python3 $THIS_DIR/$BIN.py $@ &
# export STEPMESH_DROP_RATE=1
# export STEPMESH_CPU_START_OFFSET=15
DMLC_ROLE=worker CUDA_VISIBLE_DEVICES=4,5,6,7 STEPMESH_GPU=0 python3 $THIS_DIR/$BIN.py $@ 2>&1 | tee worker.log
